...
interface FindSymbolMetadataOutput {
    fun getSymbol(symbolId: UUID): SymbolMetadata?
}
        ...
        return try {
            ...
            when (e.status.code) {
                ...
                Status.Code.DEADLINE_EXCEEDED -> {
                    log.error("Market Service timeout for symbolId=$symbolId")
                    null
                }
                else -> {
...
private fun SymbolProto.toDomain(): SymbolMetadata =
    SymbolMetadata(
        symbolId = UUID.fromString(this.symbolId),
        code = this.code,
        name = this.name,
...
| `symbol:{symbolId}` | `SymbolMetadata` (JSON) | 10분 | Market Service에서 심볼 상태 변경 시 evict (이벤트 없음, TTL 의존) |
