...
```
1.  POST /backtests { agentId, symbolId, from, to, interval }
2.  RunBacktestCommand 생성
3.  FindHistoricalCandlesOutput.getCandles(symbolId, interval, from, to)
    └ Market Service gRPC GetHistoricalCandles 호출
4.  캔들 수 검증 (0건이면 S001 에러)
5.  StreamAgentBacktestOutput.backtest(agentId, symbolId, candles)
...
| 캐시 키 | 형식 | TTL | 설명 |
|---------|------|-----|------|
| `candle:{symbolId}:{interval}:{fromDate}:{toDate}` | JSON 문자열 (캔들 배열) | 1시간 | 과거 캔들 데이터 (변경 없는 데이터) |
...
1. Redis 조회: GET candle:{symbolId}:{interval}:{from}:{to}
...
class CachedMarketCandleGrpcAdapter(
    ...
    override fun getCandles(
        ...
        val key = "candle:${symbolId}:${interval}:${from.toLocalDate()}:${to.toLocalDate()}"
        ...
        val candles = grpcAdapter.getCandles(symbolId, interval, from, to)
...
> `GET /agents/{id}/backtests/{backtestId}` (Agent Service REST)로 조회한다.
