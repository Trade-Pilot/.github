...
export default {
  ...
  theme: {
    extend: {
      ...
      boxShadow: {
        // CSS 변수 기반 — 테마별로 자동 전환
        'panel': '0 0 0 1px rgba(var(--border-strong), 0.6), 0 8px 24px rgba(var(--shadow-color), 0.35)',
        'focus': '0 0 0 3px rgba(124, 110, 245, 0.35)',
      },
    },
  },
  plugins: [],
...
```
semantic-success #06C270  → 수집 완료, 연결 성공, 긍정 피드백
semantic-warning #F5A623  → 일시정지, 재시도 대기, 주의 필요
semantic-danger  #F04452  → 오류, 실패, 삭제 액션
semantic-info    #7C6EF5  → 안내, 수집 진행 중 (= brand-500)
```
...
const variantClass: Record<ButtonVariant, string> = {
  ...
  danger:    'bg-semantic-danger hover:bg-red-500 active:bg-red-700 text-white',
...
const variantClass: Record<BadgeVariant, string> = {
  // 새 시맨틱 컬러 토큰 기반 — 배경은 10% 투명도로 은은하게
  default: 'bg-bg-elevated              text-text-secondary',
  success: 'bg-[#06C270]/10             text-[#06C270]',    // semantic-success
  error:   'bg-[#F04452]/10             text-[#F04452]',    // semantic-danger
  warning: 'bg-[#F5A623]/10             text-[#F5A623]',    // semantic-warning
  info:    'bg-brand-500/10             text-brand-400',     // semantic-info
...
export const useAuthStore = create<AuthState>()(
  persist(
    ...
    }),
    {
      ...
      partialize: (state) => ({ token: state.token }),  // token만 persist
    }
  )
)
```
...
### 16.2 Zod (런타임 API 응답 검증)

TypeScript 타입은 컴파일 타임에만 존재합니다. 백엔드가 예상치 못한 형태의 응답을 보내도 런타임에서는 감지할 수 없습니다. Zod로 런타임 검증을 추가합니다.
...
import { z } from 'zod'
import { MarketCandleCollectStatus, MarketCandleInterval, MarketType } from './types'
...
// 파서에서 Zod 사용
export const parseCollectTask = (raw: unknown) => CollectTaskSchema.parse(raw)
```

**검증 전략**: 개발 환경에서는 `parse()` (오류 throw), 프로덕션에서는 `safeParse()` (오류 로깅 후 Sentry 전송)
...
const TaskRow = React.memo(
  ...
  (prev, next) =>
    ...
    prev.task.lastCollectedAt === next.task.lastCollectedAt
)
```
...
```typescript
// 수동 에러 리포팅 (Zod 검증 실패 등)
import * as Sentry from '@sentry/react'

const parseWithMonitoring = <T>(schema: z.ZodSchema<T>, raw: unknown): T => {
  const result = schema.safeParse(raw)
  if (!result.success) {
    Sentry.captureException(result.error, {
      extra: { raw, issues: result.error.issues },
    })
    // 개발 환경에서는 throw, 프로덕션에서는 부분 데이터 반환
    if (import.meta.env.DEV) throw result.error
  }
  return result.data ?? (raw as T)
}
```

```typescript
// ErrorBoundary에 Sentry 연동
export const ErrorBoundary = Sentry.withErrorBoundary(BaseErrorBoundary, {
...
theme: {
  extend: {
    boxShadow: {
      // 다크 테마: 미세한 white 테두리 + 짙은 drop shadow 조합
...
<Checkbox
  checked={isSelected}
  onChange={(v) => toggleSelect(symbol.id, v)}
/>
...
Trade Pilot은 **데스크탑 우선(Desktop-First)** 전략을 채택합니다.
트레이딩 플랫폼 특성상 대형 화면에서의 정보 밀도가 중요하며, 모바일 지원은 1차 범위 밖입니다.

```
지원 화면 크기:
  최소 지원: 1024px (lg)  — 미만 접속 시 안내 배너
  기본 타깃: 1280px (xl)  — 4열 그리드 기준
  최적 환경: 1440px ~ 1920px
  대형:      2560px+     — max-w 제한 없음 (금융 데이터는 넓을수록 유리)
```

**사이드바 반응형**:
```tsx
// lg 미만: icon-only 축소 모드
<aside className="w-16 lg:w-60 transition-[width] duration-200 bg-bg-surface border-r border-border">
  {NAV_ITEMS.map(item => (
    <Link key={item.path} to={item.path}
          className="flex items-center gap-3 px-3 lg:px-4 py-2.5 rounded-md mx-2">
...
**소형 화면 가드**:
```tsx
// app/SmallScreenGuard.tsx
// 1024px 미만 접속 시 안내 화면으로 대체

const SmallScreenGuard = ({ children }: { children: React.ReactNode }) => {
    ...
    return (
      <div className="min-h-screen bg-bg-base flex items-center justify-center p-8">
        <div className="text-center space-y-3">
          <IconDatabase className="text-5xl text-text-tertiary mx-auto" />
          <p className="text-text-primary font-semibold">
            1024px 이상의 화면에서 이용해주세요
          </p>
          <p className="text-text-secondary text-sm">
            Trade Pilot은 데스크탑 환경에 최적화되어 있습니다
          </p>
        </div>
      </div>
    )
...
// app/App.tsx에서 루트에 적용
// <SmallScreenGuard><RouterProvider ... /></SmallScreenGuard>
```
...
- [ ] `app/SmallScreenGuard.tsx` 구현 및 `App.tsx` 루트에 적용
